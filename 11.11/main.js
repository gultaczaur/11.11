...
// tasks
// 1) hər şəhərdə nə qədər insan qaldığını tapan function qurun
// const getData = (data) => {
...
//     const usersCount = data.reduce((acc, person) => {
//         const role = person.role;
//         acc[role] = (acc[role] || 0) + 1
//         return acc;
//     }, {});
//     return Object.keys(usersCount).map(role => ({
//         role, count: usersCount[role]
//     }));
// };
// const result = getUsersCount(users);
// console.log(result);

// 3) "department": "Accounting" olanlari secib  array sekilinde ekrana cixaran func yazin
    //  const getAccounting = (data) => {
    //     return data.filter(person => person.company.department === "Accounting");
    //     };
    //     const result = getAccounting(users);
    //     console.log(result);


// 4) emailde gmail.com ile bitenleri secib bir arraya yigan func yazin
// const getGmailUsers = (data) => {
//     return data.filter(person => person.email.endsWith('@gmail.com'));
//     };
//     const result = getGmailUsers(users);
//     console.log(result);
